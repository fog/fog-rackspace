Shindo.tests('Fog::Identity::Rackspace', ['rackspace']) do

  tests('current authentication') do
    tests('variables populated').returns(200)  do
      @service = Fog::Identity::Rackspace.new :rackspace_auth_url => 'https://identity.api.rackspacecloud.com/v2.0', :connection_options => {:ssl_verify_peer => true}
  ...
  tests('reauthentication') do
    tests('should reauth with valid credentials') do
      @service = Fog::Identity::Rackspace.new  :rackspace_region => :ord
      returns(true, "auth token populated") { !@service.auth_token.nil? }
    ...
    tests('should terminate with incorrect credentials') do
      raises(Excon::Errors::Unauthorized) { Fog::Identity::Rackspace.new :rackspace_api_key => 'bad_key' }
    end
  end
